...
<BODY class="yui-skin-sam"  oncontextmenu="return true">
            ...
            <TD class="yui-skin-sam" style="FONT-SIZE: 11px;"  valign="top" width="100%" style="FONT-SIZE: 10px;">
                ...
                <div id="tvcontainer" class="yui-navset">
                    ...
                    <div class="yui-content">
                      ...
                      <?php
                        if ($_SESSION['user/level']=='superagents') {
                           ...
                           echo 'Agent';
                           echo '<br><br>';
                           echo '<input type="button" id="menubutton2" name="menubutton2_button" value="Όλοι">';
                           echo '<select id="agentsmenu" name="agentsmenu">';
                           echo '<option value="*">Όλοι</option>';
                           $result_id = mysql_query("select AgentID,Name,Remainder from agents where SAgentID=".$_SESSION['user/id']." order by Name ASC");
                           while ($row = mysql_fetch_assoc($result_id)) {
                              echo '<option value="'.$row['AgentID'].'">'.$row['Name'].' ( €'.number_format($row['Remainder'],2).' )</option>';
                           }
                      ...
                      <?php
                        if ($_SESSION['user/level']=='superagents') {
                           ...
                           echo 'Super Agent';
                           echo '<br><br>';
                           echo '<input type="button" id="menubutton3" name="menubutton3_button" value="'.$Name.' ( €'.number_format($Remainder,2).' )">';
                           echo '<select id="superagentsmenu" name="superagentsmenu"  >';

                           $Options='<option value="'.$_SESSION['user/id'].'">'.$Name.' ( €'.number_format($Remainder,2).' )</option>';


                           $result_id = mysql_query("select SAgentIDSlave,Name,Remainder from superagents,superagentsprojection where superagentsprojection.SAgentIDMaster=".$_SESSION['user/id']." AND superagents.SAgentID=superagentsprojection.SAgentIDSlave order by Name ASC");
                           while ($row = mysql_fetch_assoc($result_id)) {
                              $Options=$Options.'<option value="'.$row['SAgentIDSlave'].'">'.$row['Name'].' ( €'.number_format($row['Remainder'],2).' )</option>';
                              $AllSAgents=$AllSAgents.$row['SAgentIDSlave'].',';
                           }

                           $AllSAgents=substr($AllSAgents, 0, -1);
                           if   (mysql_num_rows($result_id) > 0) {
                                 echo '<option value="*('.$_SESSION['user/id'].','.$AllSAgents.')">Όλοι</option>';
                           }
                           echo $Options;
                           echo '</select>';
                           echo '</td>';
  ...
  <TR>
    <TD class=footerback colSpan=2>
     ...
     <Script type="text/javascript">
        ...
        var ViewFilter = '*';
        var ViewAgentFilter = '*';
        var ViewSuperAgentFilter = "<?php echo $_SESSION['user/id']?>";
        var agentdropdown =document.getElementById('agentsmenu');
        var superagentdropdown= document.getElementById('superagentsmenu');
        ...
        function computeTotal() {
         ...
         function onFind(p_oEvent) {
             ...
             var sqlquery = "query=agentbetlist&type=select&column=" + ViewAgentFilter + "&primaryKey=<?php echo ($_SESSION['user/level']=='agents') ? 'AgentID' : 'SAgentID' ?>&id="+ViewSuperAgentFilter+"&output="+ ViewFilter +"&whereID=SQL|Date(DateTime) >=@" + StartDate + "@ AND Date(DateTime) <= @"+ EndDate +"@";
		...
		var onMenuclickAgent = function (p_sType, p_aArgs) {
			...
			if (oMenuItem) {
				...
				ViewAgentFilter=oMenuItem.value;
                <?php
                    if ($_SESSION['user/level']=='superagents') {
                       echo 'oMenuButton2.set("label", oMenuItem.cfg.getProperty("text"));';
                    }
                ?>
			}
		...
		var onMenuclickSuperAgent = function (p_sType, p_aArgs) {
			...
			if (oMenuItem) {
				...
				ViewSuperAgentFilter=oMenuItem.value;

                <?php
                    if ($_SESSION['user/level']=='superagents') {
                       echo 'oMenuButton3.set("label", oMenuItem.cfg.getProperty("text"));';
                    }
                ?>

                var oMenuAgent = oMenuButton2.getMenu();

                if (ViewSuperAgentFilter.substr(0,1) != "*") {
                                 ...
                                 ajaxLoadingPanel.show();
                                 YAHOO.util.Connect.asyncRequest('GET', "../includes/dataproxy.php?query=agents&primaryKey=AgentID&type=select&output=dropdown&whereID="+ViewSuperAgentFilter, {
                                 success: function(o) {
                                        ...
                                        if (YAHOO.util.Dom.inDocument(oMenuButton2.getMenu().element)) {
                                           ...
                                           oMenuAgent.addItems([{ text:"Όλοι" , value:"*" }]);
                                           for (var i=0;i<r.DataSet.items.length;i++) {

                                                  oMenuAgent.addItems([{ text:r.DataSet.items[i].label , value:r.DataSet.items[i].value }]);
                                           ...
                                           agentdropdown.options.length = 0;


                                           var data = '{ "text":"Όλοι" , "value":"*" }' + ( (r.DataSet.items.length) > 0 ? ',' : '' );


                                           for (var i=0;i<r.DataSet.items.length;i++) {

                                                  data = data + '{ "text":"' + r.DataSet.items[i].label + '" , "value":"' + r.DataSet.items[i].value + '" }' + ( (i<r.DataSet.items.length-1) ? ',' : '' );

                                           }
                                           data = '['+data+']';
                                           oMenuAgent.itemData = YAHOO.lang.JSON.parse(data);

                                           //if no data is returned then add only the option "All";
                                           if (i==0)
                                               agentdropdown.options.length = 1;
                                           else
                                               agentdropdown.options.length = i;
                                       ...
                                       agentdropdown.selectedIndex=0;
                                       oMenuButton2.set("label", "Όλοι");
                                       ViewAgentFilter='*';
        ...
        <?php
            if ($_SESSION['user/level']=='superagents') {
		        ...
		        echo 'var oMenuButton2 = new YAHOO.widget.Button("menubutton2",{ type: "menu", menu: "agentsmenu" });';
                echo 'oMenuButton2.getMenu().subscribe("click", onMenuclickAgent);';

		        echo 'var oMenuButton3 = new YAHOO.widget.Button("menubutton3",{ type: "menu", menu: "superagentsmenu" });';
                echo 'oMenuButton3.getMenu().subscribe("click", onMenuclickSuperAgent);';
        ...
        ?>
          ...
          YAHOO.example.userslist =  function() {
            ...
            var StatusCanceled = function(elCell, oRecord, oColumn, oData) {
            ...
            var PossibleProfitFormatter = function(elCell, oRecord, oColumn, oData) {



                    if ( (oRecord.getData('Canceled') == 0)) {
                      //call the original formatter
                      YAHOO.widget.DataTable.formatCurrency.call(DataTable,elCell, oRecord, oColumn, oData);
                    } else {

                      YAHOO.widget.DataTable.formatCurrency.call(DataTable,elCell, oRecord, oColumn, 0);
                    }

            };

            var FinalProfitFormatter = function(elCell, oRecord, oColumn, oData) {
           ...
           var myColumnDefs = [
                ...
                {key:"id_u", label:"ΚΩΔ. ΔΕΛΤΙΟΥ", width:90, formatter:YAHOO.widget.DataTable.formatText,sortable:true,hidden:false },
                {key:"id_s", hidden:true },
                {key:"SName", label:"SAGENT", width:90, formatter:YAHOO.widget.DataTable.formatText,sortable:true },
                ...
                {key:"PossibleProfit", label:"ΠΙΘ. ΚΕΡΔΟΣ",  width:90, formatter:PossibleProfitFormatter,  sortable:true, sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}},
                {key:"FinalUserProfit", label:"ΚΕΡΔΗ ΠΑΙΚΤΗ", width:90, formatter:FinalProfitFormatter, sortable:true, sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}},
                {key:"Status", label:"ΚΑΤΑΣΤΑΣΗ",width:70,  formatter:StatusFormatter, sortable:true},
                {key:"Canceled", label:"ΑΚΥΡΩΜΕΝΟ",width:70,  formatter:StatusCanceled, sortable:true}
            ...
            var oConfigs = {
                    ...
                    initialRequest:"query=agentbetlist&type=select&column=" + ViewAgentFilter + "&primaryKey=<?php echo ($_SESSION['user/level']=='agents') ? 'AgentID' : 'SAgentID' ?>&id="+ViewSuperAgentFilter+"&output="+ ViewFilter +"&whereID=SQL|Date(DateTime) >=@" + StartDate + "@ AND Date(DateTime) <= @"+ EndDate +"@",
                    // Sorting and pagination will be routed to the server via generateRequest
                    /*
                    sortedBy : {
                        key: "UserBetSynopsisID",
