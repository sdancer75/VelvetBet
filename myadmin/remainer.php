     ...
     <Script type="text/javascript">
        ...
        var ViewAgentFilter = '*';
         ...
         function onFind(p_oEvent) {
             ...
             var sqlquery = "query=allremainer&type=select&whereID=SQL|Date(DateTime) >=@" + StartDate + "@ AND Date(DateTime) <= @"+ EndDate +"@";
          ...
          YAHOO.example.userslist =  function() {
            ...
            var NameFormatter = function(elCell, oRecord, oColumn, oData) {
                    ...
                    if (oData == 'SAGENT_TOTAL')
                        ...
                        elCell.innerHTML = 'ΟΝΟΜΑ';
                    else if (oData == 'TOTAL')
                        elCell.innerHTML = 'ΓΕΝΙΚΟ ΣΥΝΟΛΟ';
                    else
                        elCell.innerHTML = oData;

            };

            var WinLoseFormatterFinal  = function(elCell, oRecord, oColumn, oData) {

                    if ( (oRecord.getData('BetSum') != -1)
                          && (oRecord.getData('Name') != 'EMPTY') && (oRecord.getData('Name') != 'NAME') ) {
                        ...
                        if (eval(oData) < 0) {
                             ...
                             if (oRecord.getData('Name') == 'SAGENT_TOTAL')
                                 YAHOO.util.Dom.replaceClass(elCell.parentNode, "", "TotalWin");
                             else
                                 YAHOO.util.Dom.replaceClass(elCell.parentNode, "", "Win");
                        }

                        elCell.innerHTML = WinLoseMoney;
                    } else if  (oRecord.getData('Name') == 'NAME') {

                      elCell.innerHTML = "ΚΑΘΑΡΟ";

                    }
            };

            var WinLoseFormatterCommission  = function(elCell, oRecord, oColumn, oData) {
            ...
            var BetFormatter  = function(elCell, oRecord, oColumn, oData) {

                    if ( (oRecord.getData('BetSum') != -1)
                                    && (oRecord.getData('Name') != 'EMPTY') && (oRecord.getData('Name') != 'NAME')) {

                        Money = '€'+eval(oData).toFixed(2);
                        elCell.innerHTML = Money;
                    } else if  (oRecord.getData('Name') == 'NAME') {

                      elCell.innerHTML = "Σ.ΚΛΕΙΣΤΩΝ";

                    }
            };

            var BetSumFormatter  = function(elCell, oRecord, oColumn, oData) {
                    ...
                    if ( (oRecord.getData('BetSum') != -1)
                      ...
                      elCell.innerHTML = "ΑΡ.ΚΛΕΙΣΤΩΝ";

                    }
            };

            var MiktoFormatter  = function(elCell, oRecord, oColumn, oData) {
                    ...
                    if ( (oRecord.getData('BetSum') != -1)
                      ...
                      elCell.innerHTML = "ΜΕΙΚΤΟ";

                    }
            };

            var CommissionFormatter  = function(elCell, oRecord, oColumn, oData) {
                    ...
                    if ( (oRecord.getData('BetSum') != -1)
                      ...
                      elCell.innerHTML = "ΠΡΟΜ. AGENT";

                    }
            };

            var ProfitFormatter  = function(elCell, oRecord, oColumn, oData) {
                    ...
                    if ( (oRecord.getData('BetSum') != -1)
                      ...
                      elCell.innerHTML = "Σ.ΚΕΡΔΙΣΜΕΝΩΝ";

                    }
            };


            var OpenBetSumFormatter  = function(elCell, oRecord, oColumn, oData) {
                    ...
                    if ( (oRecord.getData('BetSum') != -1)
                      ...
                      elCell.innerHTML = "ΑΡ.ΑΝΟΙΧΤΩΝ";

                    }
            };

            var OpenBetFormatter  = function(elCell, oRecord, oColumn, oData) {
            ...
            var TotalFormatter = function(elTr, oRecord) {
                ...
                }  else if (oRecord.getData('BetSum') == -1) {

                    YAHOO.util.Dom.addClass(elTr, "BetTypeAgentLabel");

                }  else if (oRecord.getData('Name') == 'EMPTY') {

                    YAHOO.util.Dom.addClass(elTr, "EmptyLine");

                }  else if (oRecord.getData('Name') == 'NAME') {

                    YAHOO.util.Dom.addClass(elTr, "Title");

                }  else if (oRecord.getData('Name') == 'TOTAL') {

                    YAHOO.util.Dom.addClass(elTr, "Total");
                }
           ...
           var myColumnDefs = [
                ...
                {key:"Name", label:"ΟΝΟΜΑ", width:100, formatter:NameFormatter,sortable:false },
                {key:"Bet", label:"Σ.ΚΛΕΙΣΤΩΝ", width:100, formatter:BetFormatter, sortable:false},
                {key:"Profit", label:"Σ.ΚΕΡΔΙΣΜΕΝΩΝ",  width:100, formatter:ProfitFormatter,  sortable:false},
                {key:"Commission", label:"ΠΡΟΜ. AGENT", width:100, formatter:CommissionFormatter, sortable:false},
                {key:"Mixed", label:"ΜΕΙΚΤΟ",width:100,  formatter:MiktoFormatter, sortable:false},
                {key:"Commission_SAgent", label:"ΠΡΟΜ. SA",width:100,  formatter:WinLoseFormatterCommission, sortable:false},
                {key:"Final", label:"ΚΑΘΑΡΟ",width:100,  formatter:WinLoseFormatterFinal, sortable:false},
                {key:"BetSum", label:"ΑΡ.ΚΛΕΙΣΤΩΝ", width:100, formatter:BetSumFormatter ,sortable:false},
                {key:"OpenBetSum", label:"ΑΡ.ΑΝΟΙΧΤΩΝ", width:100, formatter:OpenBetSumFormatter ,sortable:false},
                {key:"OpenBet", label:"Σ.ΑΝΟΙΧΤΩΝ", width:100, formatter:OpenBetFormatter ,sortable:false}
            ...
            var oConfigs = {
                    ...
                    initialRequest:"query=allremainer&type=select&whereID=SQL|Date(DateTime) >=@" + StartDate + "@ AND Date(DateTime) <= @"+ EndDate +"@",
